...
<audio id="dingAudio" preload="none">
...
document.addEventListener('livewire:init', () => {
    ...
    const dingFile = "http://bbjj.qwaiting.com/tenancy/assets/audio/Ding-noise.mp3";
    let dingSound = null;
    ...
    function playDing() {
      ...
      document.getElementById('manualPlayAudio').addEventListener('click', function() {
        playDing();
    ...
    document.addEventListener('DOMContentLoaded', () => {
        const btn = document.getElementById("manualPlayAudio");
        if (btn) {
            btn.addEventListener('click', () => playDing());
        }
    });

    // Livewire announcement trigger
    if (window.Livewire) {

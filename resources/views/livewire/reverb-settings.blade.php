<div class="p-4 dark:bg-gray-900 bg-transparent">
    <h2 class="text-xl font-semibold mb-4">{{ __('setting.Reverb Settings') }}</h2>
    <div class="rounded-lg shadow border p-4 border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03]">
        <form wire:submit.prevent="save">
            ...
            <div class="mt-6 grid grid-cols-2 gap-4">
                <!-- Custom Labels -->
                @foreach([
                    'key' => __('setting.Reverb App Key'),
                    'secret' => __('setting.Reverb Secret'),
                    'app_id' => __('setting.Reverb APP ID'),
                    'host' => __('setting.Reverb Host'),
                    'port' => __('setting.Reverb Port'),
                    'scheme' => __('setting.Reverb Scheme'),
                ] as $field => $label)
                    <div>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        window.Livewire.on('reverb-settings-updated', count => {
            Swal.fire({
                title: "Success!",
                text: "Reverb Settings Updated!",
                icon: "success"
            });
        });
    });

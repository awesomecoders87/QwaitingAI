<div>
   @if($mainpage)
   <div class="main-page mx-auto max-w-screen-2xl p-4 md:p-4">
      <style>       
      ...
      <h2 class="fi-section-header-heading page-title text-xl font-semibold dark:text-white">{{ __('setting.Ticket Generate Settings') }}</h2>
      
      
      <div class="flex justify-between gap-6" style="--cols-default: repeat(1, minmax(0, 1fr)); --cols-lg: repeat(2, minmax(0, 1fr));">
         <!-- <h3 class="fi-section-header-heading text-xl font-semibold leading-2 0 dark:text-white mb-4">Ticket hours</h3> -->
         <div class="flex items-center mb-4">
         <label class="switch">
            <input type="checkbox" wire:click="toggle" wire:model="isEnabled">
            <span class="slider round"></span>
         </label>
         <span class="ml-3 font-medium text-gray-900 dark:text-white" style="margin:10px">
            {{ $isEnabled ? __('setting.Enabled Ticket Time Slot') : __('setting.Disabled Ticket Time Slot') }}
         </span>
      </div>
      @if($isEnabled)
         <div class="">
            <button class="flex gap-x-2 items-center justify-center px-3 py-2 text-sm font-medium text-white transition rounded-lg bg-brand-500 shadow-theme-xs hover:bg-brand-600" wire:click="showEditModal">
      ...
      <div class="p-3  h-full bg-white rounded-lg shadow border-gray-200 border col-md-6 gap-3 px-6 py-4 col-[--col-span-default] dark:bg-white/[0.03]  dark:border-gray-700">
            <h3 class="fi-section-header-heading text-base font-semibold  dark:text-white">Weekly hours</h3>
            <ul class="mt-3">
      ...
      <div class="w-full px-2.5 xl:w-1/2 mb-3">
         <div class="p-3 rounded-lg shadow  border h-full bg-white border-gray-200 col-md-6 gap-3 px-6 py-4 col-[--col-span-default] dark:bg-white/[0.03]  dark:border-gray-700">
      ...
      <div class="grid justify-between gap-2 mt-4" style="--cols-default: repeat(1, minmax(0, 1fr)); --cols-lg: repeat(1, minmax(0, 1fr));">
         ...
         <p class="dark:text-white">{{ __('setting.Configure basic functionality around your bookings.') }}</p>
      </div>
      <ul class="mt-3 border shadow rounded border-gray-300 dark:bg-white/[0.03]  dark:border-gray-700">
      <li class="flex gap-3 p-6 bg-white border-b border-gray-300 justify-items-center justify-between cursor-pointer  dark:border-gray-700 dark:bg-white/[0.03] dark:hover:bg-gray-700">
         <a href="{{ url('ticket-screen-settings') }}" class="flex justify-between w-full">   
            <div class="">
               <h3 class="fi-section-header-heading text-base font-semibold dark:text-white">{{ __('setting.Ticket Screen Settings') }}</h3>
               <!-- <p class="text-gray-600 dark:text-white text-xs">Allow registration only if the customer is within a certain radius from your location.</p> -->
            </div>
           <div class="flex">
         ...
         {{-- <li class="flex gap-3 p-6 bg-white border-b border-gray-300 justify-items-center justify-between  dark:border-gray-700 dark:bg-white/[0.03] dark:hover:bg-gray-700">
            <div class="">
               <h3 class="fi-section-header-heading text-base font-semibold dark:text-white">
                  {{ __('setting.Allow customers to register multiple times') }}
                </h3>
               <p class="text-gray-600 dark:text-white text-xs">{{ __('setting.Customers will be able to register for the waitlist multiple times.') }}</p>
            </div>
            <div class="flex">
               <span>
                  <label class="switch">
                     <input type="checkbox" wire:click="customRegisterMultipleToggle" wire:model="customersRegisterMultiple">
                     <span class="slider round"></span>
                  </label>
               </span>
            </div>
         ...
         {{-- <li class="flex gap-3 p-6 bg-white border-b border-gray-300 justify-items-center justify-between cursor-pointer  dark:border-gray-700 dark:bg-white/[0.03] dark:hover:bg-gray-700" wire:click="showPage('waitlistLimit')">
            <div class="">
               <h3 class="fi-section-header-heading text-base font-semibold dark:text-white">{{ __('setting.Limit max number of visits on the waitlist') }}</h3>
               <p class="text-gray-600 dark:text-white text-xs">{{ __('setting.Disable the waitlist for new registrations when a certain number of visits has been reached.') }}</p>
            </div>
            <div class="flex">
               <span><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24">
                     <path d="M10.707 17.707 16.414 12l-5.707-5.707-1.414 1.414L13.586 12l-4.293 4.293z"></path>
         ...
         <li class="flex gap-3 p-6 bg-white border-b border-gray-300 justify-items-center justify-between cursor-pointer  dark:border-gray-700 dark:bg-white/[0.03] dark:hover:bg-gray-700" wire:click="showPage('geofence')">
            <div class="">
               <h3 class="fi-section-header-heading text-base font-semibold dark:text-white">{{ __('setting.Geofence') }}</h3>
               <p class="text-gray-600 dark:text-white text-xs">{{ __('setting.Allow registration only if the customer is within a certain radius from your location.') }}</p>
            </div>
            <div class="flex">
               <span><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24">
                     <path d="M10.707 17.707 16.414 12l-5.707-5.707-1.414 1.414L13.586 12l-4.293 4.293z"></path>
         ...
         <li class="flex gap-3 p-6 border-b bg-white border-gray-300 justify-items-center justify-between  dark:border-gray-700 dark:bg-white/[0.03] dark:hover:bg-gray-700">
            <div class="">
               <h3 class="fi-section-header-heading text-base font-semibold dark:text-white">{{ __('setting.Allow customer to leave the line') }} </h3>
               <p class="text-gray-600 dark:text-white text-xs">{{ __('setting.Customers are allowed to leave the waitlist.') }}</p>
            </div>
            <div class="flex">
               <span>
                  <label class="switch">
      ...
      @if($showModal)
      <!-- Edit Modal -->
      <div class="fixed left-0 top-0 z-99999 flex h-screen w-full flex-col overflow-y-auto overflow-x-hidden p-5 dark:bg-gray-900 lg:p-10" style="background: rgba(0,0,0,0.25);">
         ...
         <div class="p-5 modal-close-btn inset-0  m-auto w-full max-w-2xl bg-white-400/50 bg-white rounded-xl relative">
                  ...
                  <div>
                     ...
                     <div class="mt-4 space-y-4">
                        ...
                        <div wire:key="days-{{ $slotIndex }}">
                           <div class="mt-3 flex gap-4">
                              <input type="date" onclick="this.showPicker()" wire:model="customSlots.{{ $slotIndex }}.selected_date" class="w-full border p-2 rounded-md border-gray-400 ">
                           ...
                           <div class="mt-3 flex gap-4">
                              <input type="time" onclick="this.showPicker()" wire:model="customSlots.{{ $slotIndex }}.start_time" class="w-full border p-2 rounded-md border-gray-400" placeholder="Start Time">
                              <input type="time" onclick="this.showPicker()" wire:model="customSlots.{{ $slotIndex }}.end_time" class="w-full border p-2 rounded-md border-gray-400" placeholder="End Time">
                           ...
                           <div class="mt-3 flex gap-4">
                              <input type="time" onclick="this.showPicker()" wire:model="customSlots.{{ $slotIndex }}.day_interval.{{$Index}}.start_time" class="w-full border p-2 rounded-md border-gray-400" placeholder="Start Time">
                              <input type="time" onclick="this.showPicker()" wire:model="customSlots.{{ $slotIndex }}.day_interval.{{$Index}}.end_time" class="w-full border p-2 rounded-md border-gray-400" placeholder="End Time">
                              <button type="button" wire:click="removeCustomSlot({{ $slotIndex }},{{ $Index }})" class="text-red-500"> ✖</button>
                           </div>
                           @endforeach
                           <div class="flex mt-3 gap-2">
                              ...
                              <button type="button" wire:click="deleteCustomSlot({{ $slotIndex }})" class="px-4 py-3 text-sm font-medium text-white rounded-lg bg-brand-500 shadow-theme-xs hover:bg-brand-600 text-sm px-2.5 py-1.5">x {{ __('setting.Remove') }}</button>
                           </div>
                        </div>
                        @endforeach
                        <div>
                           <button type="button" wire:click="addNextCustomSlot()" style="--c-400:var(--primary-400);--c-500:var(--primary-500);--c-600:var(--primary-600);" class="fi-btn relative grid-flow-col items-center justify-center font-semibold outline-none transition duration-75 focus-visible:ring-2 rounded-lg fi-color-custom fi-btn-color-primary fi-color-primary fi-size-sm fi-btn-size-sm gap-1 px-2.5 py-1.5 text-sm inline-grid shadow-sm bg-custom-600 text-black hover:bg-custom-500 focus-visible:ring-custom-500/50 dark:bg-custom-500 dark:hover:bg-custom-400 dark:focus-visible:ring-custom-400/50">+ {{ __('setting.Add Next Date Slot') }}</button>
                        </div>
                     </div>
            ...
            <div class="mt-6 flex justify-end space-x-3 gap-3 mb-3">
   ...
   <div class="p-4">
      <div class="inside-page-heading flex items-center gap-4">
         ...
         <h3 class="fi-section-header-heading text-xl font-semibold leading-2 text-gray-950 dark:text-white">{{ __('setting.Limit max number of visits on the waitlist') }}</h3>
      </div>
      <div class="p-4 md:p-4 rounded-lg max-w-screen-md border border-gray-200 bg-white shadow mt-4">
   ...
   <div class="p-4">
      <div class="inside-page-heading flex items-center gap-4">
         ...
         <h3 class="fi-section-header-heading text-xl font-semibold leading-2 dark:text-white">{{ __('setting.Geofence') }}</h3>
      </div>
      <div class="mt-4">
      @livewire('App\Livewire\GeofenceComponent',['teamId' => $teamId, 'locationId' => $locationId,'slotType'=>$type])
         </div>
   </div>
   @elseif($ticketOtherSettings)
   <div>
      <div class="inside-page-heading flex items-center gap-4">
         ...
         <h3 class="fi-section-header-heading text-2xl font-semibold leading-2 text-gray-950 dark:text-white">{{ __('setting.Geofence') }}</h3>
      </div>
      @livewire('App\Livewire\GeofenceComponent',['teamId' => $teamId, 'locationId' => $locationId,'slotType'=>$type])
   </div>

document.addEventListener("livewire:init", () => {
    ...
    Livewire.on("desktop-notify", (response) => {
        let e = response;
        console.log(e);
        Livewire.dispatch("desktop-notification", {
            token_notify: e.token_notify,
        });
    });
    ...
    Livewire.on("swal:saved-queue", (response) => {
        ...
        Swal.fire({
            ...
            if (
                result.isConfirmed ||
                result.dismiss === Swal.DismissReason.outside
            ) {
                Swal.close();
    ...
    Livewire.on("booked-print", (response) => {
        let htmlData = response.html || (response[0] && response[0].html);
        let html = `<!DOCTYPE html>
            <html>
            <head>
                <meta charset="utf-8">
    ...
    function getVoiceMessage(key) {
        return (
            voiceMessages[key] || { lang: "en-US", voice: "Google US English" }
    ...
    Livewire.on("event-datepicker", (response) => {
        ...
        if (
            (response[0].closed_days &&
                response[0].closed_days.includes(today.getDay())) ||
            response[0].disabled_dates.includes(
            ...
            while (true) {
                ...
                if (
                    !response[0].closed_days.includes(defaultDate.getDay()) &&
                    !response[0].disabled_dates.includes(
        ...
        flatpickr(datePicker, {
            ...
            disable: [
                ...
                ...response[0].disabled_dates.map(
                    (dateString) => new Date(dateString),
                ),
            ],
            onChange: function (selectedDate, dateStr, instance) {
                console.log(dateStr);
                Livewire.dispatch("update-appointment-time", {
                    value: dateStr,
                });
            },
        });
        Livewire.dispatch("update-appointment-time", {
            value: defaultDate.toLocaleDateString("en-CA"),
        });
    });
});
...
document.addEventListener("DOMContentLoaded", function () {
    const closeAllDropdowns = () => {
        document
            .querySelectorAll(".dropdown-menu.fixed-dropdown")
            .forEach((menu) => {
...
document.addEventListener("livewire:init", () => {
    Livewire.hook(
        "request",
        ({ fail, respond, payload, succeed, resolve, reject, options }) => {

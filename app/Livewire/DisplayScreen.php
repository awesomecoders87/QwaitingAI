   ...
   public function getcallsdetail()
{
    ...
    public function pushLiveQueue($event)
    {
        ...
        if (!empty($event['queue'])) {
            ...
            if( $this->currentTemplate->type === "Counter" && !empty($event['queue']['counter_id']) && in_array($event['queue']['counter_id'], $this->counterID)){
                $checktype= true;
            }
            
            if( $this->currentTemplate->type === "Counter" && !empty($event['queue']['forward_counter_id']) && in_array($event['queue']['forward_counter_id'], $this->counterID)){
                $checktype= true;
            }

            if( $this->currentTemplate->type === "Category" && !empty($event['queue']['category_id']) && in_array($event['queue']['category_id'], $this->categoryID)){
                $checktype= true;
            }

            if( $this->currentTemplate->type === "Category" && !empty($event['queue']['transfer_id']) && in_array($event['queue']['transfer_id'], $this->categoryID)){
                $checktype= true;
            }
            if (isset($event['queue']['is_missed']) && !empty($event['queue']['is_missed']) && $event['queue']['is_missed'] == 1) {
                    $checktype = false;
                }
            ...
            if ($event['queue']['status'] == Queue::STATUS_PROGRESS && !empty($event['queue']['called_datetime'])) {
                ...
                if (!empty($counterName)) {
                    ...
                    if ($voice && ($voice['lang'] != DisplaySettingModel::DEFAULT_EN_LANG)  && $voice['lang'] == 'es-ES') {
                        $speech  = 'tiquete number ' . $event['queue']['start_acronym'] . $event['queue']['token'].$anounce_text . $counterName;
                    } else {
                        $speech  = 'token number ' . $event['queue']['start_acronym'] . $event['queue']['token'] . $anounce_text . $counterName;
                    }
            ...
            $activeid =$event['queue']['id'];
            if(!empty($event) &&  $this->currentTemplate->display_behavior == 2){

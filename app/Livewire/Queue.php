...
class Queue extends Component
{
    ...
    public $ticket_note_level_third;

    public $allowed_Countries = [];
    public $country_phone_mode = 1;
    public $enable_doc_file_field = true;
    ...
    public function rules()
{
    ...
    public function messages()
    {
        ...
        $messages['docFile.mimes'] = 'Unsupported file format. Allowed: PDF, JPG, PNG, DOC, DOCX, XLS, XLSX, CSV.';
        $messages['docFile.mimetypes'] = 'Unsupported file format. Allowed: PDF, JPG, PNG, DOC, DOCX, XLS, XLSX, CSV.';
        $messages['docFile.max']   = 'File too large. Maximum size allowed is 2MB.';
    ...
    public function updatedDocFile()
{
 ...
 if ($this->siteDetails->category_estimated_time == SiteDetail::STATUS_YES) {
            ...
            $queueStorage->actual_staff_assign_id = $actualAgent ?? null;
   ...
   private function isWithinTimeSlot($slotType, $currentDate, $currentDay, $currentTime, $categoryId=null, $userId=null)
     ...
     private function isWithinRadius($lat, $lng, $radius = 100)
    {
        ...
        if (isset($location_detail) && !empty($location_detail->latitude) && !empty($location_detail->longitude)) {
            ...
            $distance = (float) $this->calculateDistance($centralLat, $centralLng, $lat, $lng);

            // Convert radius to meters (radius is already in meters, no conversion needed)
            $radiusInMeters = (float) ($radius ?: 0);

            // Log the values for debugging
            // dd(round($distance) . ' / ' . round($radiusInMeters));

            // Compare the distance with the radius using rounded values (distance and radius are already cast to float)

...
class SingpassSetting extends Component
{
    ...
    public function mount(): void
    {
        ...
        // Clean URLs — no query params (tenant identified by domain)
        $this->jwksUrl = url('/sp/jwks');
        $this->callbackUrl = url('/sp/callback');
    }
    ...
    public function save(): void
    {
        ...
        $setting->fill([
            ...
            'signing_private_key' => $signingPrivatePem, // mutator encrypts
            'signing_public_key' => $signingPublicPem,
            'enc_private_key' => $encPrivatePem, // mutator encrypts
            'enc_public_key' => $encPublicPem,
            'keys_generated_at' => now(),
            'created_by' => Auth::id(),
        ])->save();

        // ── Clear JWKS cache ───────────────────────────────────────────────────
        Cache::forget('singpass_jwks_' . $this->teamId . '_' . $this->locationId);

        // ── Refresh UI state ───────────────────────────────────────────────────
        $this->signing_key_exists = true;
        $this->enc_key_exists = true;
        $this->keys_generated_at = now()->format('d M Y H:i');

        session()->flash('success', 'Settings saved. Copy the JWKS URL and register it in the Singpass Developer Portal.');
